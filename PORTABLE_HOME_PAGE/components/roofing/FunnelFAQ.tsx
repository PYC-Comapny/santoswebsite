...
import { HelpCircle, Shield, Clock, FileCheck, Hammer } from 'lucide-react';
...
const defaultFaqs = [
    ...
    },
    {
        ...
        answer: (
            <>
                Absolutely. We are fully licensed nearby (<Link href="/" prefetch={false} className="hover:text-premiumAccent underline transition-colors">LICENSE: PA006234</Link>) and carry both comprehensive liability and workers&apos; compensation insurance for your protection. With 25+ years of experience in Erie, we operate with maximum integrity on every project.
            </>
        )
    },
    {
...
export default function FunnelFAQ({ items }: FunnelFAQProps) {
    ...
    return (
        <section ref={containerRef} className="py-12 relative overflow-hidden bg-white">
                                ...
                                >
                                    <AccordionTrigger className="hover:no-underline py-6">
                                        <div className="flex items-center gap-3 md:gap-4 text-left">
                                            <div className="w-8 h-8 md:w-10 md:h-10 shrink-0 rounded-xl bg-white shadow-sm border border-slate-200 flex items-center justify-center text-premiumAccent group-data-[state=open]:bg-premiumAccent group-data-[state=open]:text-white transition-all duration-300">
                                                {faq.icon}
                                            </div>
                                            <span className="text-base md:text-xl font-black text-slate-900 uppercase tracking-tighter">
                                                {faq.question}
                                            </span>
                                        </div>
                                    </AccordionTrigger>
